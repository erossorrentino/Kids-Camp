...
function buildHeliMesh() {
  ...
  const body = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x3a3f46, roughness: 0.4, metalness: 0.4 })
  );
  ...
  const tailBoom = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x3a3f46 })
  );
...
export class Helicopter {
  constructor(scene, position) {
    const { group, rotor, tailRotor } = buildHeliMesh();
    this.mesh = group;
    this.rotor = rotor;
  ...
  update(dt, input) {
    ...
    this.pitch = THREE.MathUtils.lerp(this.pitch, targetPitch, Math.min(1, HELI.pitchRollRate * dt));
    this.roll = THREE.MathUtils.lerp(this.roll, targetRoll, Math.min(1, HELI.pitchRollRate * dt));
    ...
    this.velocity.y += (lift + (ascend - descend) * HELI.ascendSpeed * this.rotorSpeed) * dt;
    this.velocity.y *= 1 - HELI.drag * dt;

    const fwd = new THREE.Vector3(Math.sin(this.heading), 0, Math.cos(this.heading));
    const right = new THREE.Vector3(fwd.z, 0, -fwd.x);
    const accel = 14 * this.rotorSpeed;
...
function buildJetMesh() {
  ...
  const fuselage = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x5b6b78, roughness: 0.3, metalness: 0.6 })
  );
...
export class Jet {
  constructor(scene, position) {
    const { group, flame } = buildJetMesh();
    this.mesh = group;
    this.flame = flame;
    this.mesh.position.copy(position);
    scene.add(this.mesh);
  ...
  update(dt, input) {
    ...
    this.speed = THREE.MathUtils.clamp(this.speed + throttle * JET.throttleAccel * dt, 0, JET.maxSpeed);

    const targetRoll = rollIn * 0.9;
    this.roll = THREE.MathUtils.lerp(this.roll, targetRoll, Math.min(1, JET.rollRate * dt));
    const targetPitch = pitchIn * 0.6;
    this.pitch = THREE.MathUtils.lerp(this.pitch, targetPitch, Math.min(1, JET.pitchRate * dt));

    // banking contributes to turn rate (arcade bank-to-turn), plus direct yaw input
    this.heading += (yawIn * JET.yawRate - this.roll * 0.8) * dt;

    this.stalling = this.speed < JET.minLiftSpeed;
    const speedFrac = this.speed / JET.maxSpeed;
